...
def Broadcast(request):
    user = request.user
  #  modules = Course.objects.filter(user=user)

    course_select = request.GET.get('course_select')
    body = request.GET.get('message')

    #if course_select and body:
   #     modules = Course.objects.get(id=course_select)

    #    if user != modules.user:
       #     return HttpResponseForbidden()
        #else:
     #       for student in modules.enrolled.all():
...
def PeopleWeCanMessage(request):
    ...
    return render(request, 'chat/can_message_list.html', context)
...
def UserSearch(request):
    ...
    #modules = Course.objects.filter(enrolled=user)

    #if query and course_select:
     #   students = Course.objects.filter(Q(id=course_select) & Q(enrolled__username__icontains=query)).values_list(
        #    'enrolled__username', flat=True)
